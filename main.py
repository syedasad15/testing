...
#             )
#         else:
#             if not st.session_state.websearch_enabled:
#                 # 🔹 Default: format text, no clickable links
#                 html_msg = re.sub(r"(?m)^([A-Z][a-z]+):", r"<strong>\1:</strong>", msg["message"])
#                 html_msg = html_msg.replace("\n", "<br>")
#                 st.markdown(
#                     f'<div class="chat-assistant"><strong>⚖️ Assistant:</strong><br>{html_msg}</div>',
#                     unsafe_allow_html=True,
#                 )
#             else:
#                 # 🔹 Websearch: clickable links, all inside one bubble
#                 st.markdown(
#                     f"""
#                     <div class="chat-assistant">
#                         <strong>⚖️ Assistant:</strong><br>
#                         {msg["message"]}
#                     </div>
#                     """,
#                     unsafe_allow_html=True,
...
#             if result.get("error"):
#                 st.sidebar.error(result["error"]["message"])
#             else:
...
# # ---------- Initialize per-user chats ----------
# user_id = st.session_state.user.id
# if "chats" not in st.session_state:
#     # Fetch user's chats from database
#     data = supabase.table("chats").select("*").eq("user_id", user_id).execute()
#     st.session_state.chats = {}
#     st.session_state.chat_titles = {}
#     if data.data:
#         for c in data.data:
#             cid = c["id"]
#             st.session_state.chats[cid] = c["messages"]
#             st.session_state.chat_titles[cid] = c["title"]
